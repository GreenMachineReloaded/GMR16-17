...
public class FirstBeaconBlue extends LinearOpMode{
    public void runOpMode() throws InterruptedException {
        ...
        move.colorWhiteDrive(Directions.DLeftUp, .5, ColorSensors.whichColorSensor.GROUNDLEFT);
        telemetry.addData("finished move.colorwhiteDrive", 0);
        telemetry.update();

        move.ProxDrive(Directions.Forward, .5, .7);
        telemetry.addData("finished move.colorwhiteDrive", 0);
        telemetry.addData("finished move.proxDrive", 1);
        telemetry.update();

        move.colorDriveRedBlue(Directions.StrafeRight, .5, ColorSensors.whichColorSensor.BEACON, ColorSensors.whichColor.BLUE);
        telemetry.addData("finished move.colorwhiteDrive", 0);
        telemetry.addData("finished move.proxdrive", 1);
